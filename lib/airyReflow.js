...
/// Marker spans exempt from reflowing: clients render each span as one
/// native element ([CHECK] callout; blocks_v1 [KEY]/[EX] cards and the
/// line-structured [Q] quiz whose interior must not gain paragraph breaks).
const SPAN_TOKENS = [
  { open: CHECK_OPEN, close: CHECK_CLOSE },
  { open: '[KEY]', close: '[/KEY]' },
  { open: '[EX]', close: '[/EX]' },
  { open: '[Q]', close: '[/Q]' },
];
const MAX_SPAN_TOKEN_LEN = Math.max(...SPAN_TOKENS.flatMap((p) => [p.open.length, p.close.length]));
...
function createReflow({ limit }) {
  let buf = '';
  let inFence = false;
  // Non-null while inside an exempt marker span — holds the close token
  // we're scanning for ([/CHECK], [/KEY], [/EX], [/Q]).
  let spanClose = null;
  ...
  function push(chunk) {
    ...
    scan: while (i < buf.length) {
      ...
      if (!inFence && buf[i] === '[') {
        const tail = buf.slice(i, i + MAX_SPAN_TOKEN_LEN);
        if (spanClose === null) {
          const pair = SPAN_TOKENS.find((p) => tail.startsWith(p.open));
          if (pair) {
            spanClose = pair.close;
            emit(pair.open); i += pair.open.length;
            continue;
          }
          // Possible partial open tag at the very tail — wait for more data.
          if (rest < MAX_SPAN_TOKEN_LEN &&
              SPAN_TOKENS.some((p) => p.open.length > tail.length && p.open.startsWith(tail))) break;
        } else {
          if (tail.startsWith(spanClose)) {
            emit(spanClose); i += spanClose.length;
            spanClose = null;
            continue;
          }
          // Possible partial close tag at the very tail — wait for more data.
          if (rest < MAX_SPAN_TOKEN_LEN &&
              spanClose.length > tail.length && spanClose.startsWith(tail)) break;
        }
        emit('['); classify('['); i += 1;
        continue;
      }
