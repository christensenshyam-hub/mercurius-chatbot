 ...
 *         getAdminStats({ days: 7 }) (ONE call: today's figures are the
 *         window's last perDay row, the 7-day ones its totals) →
 *         notify('digest', formatDigest(digestStatsFromAdminStats(stats)),
 ...
 *           REPORT_RETENTION_DAYS         = 180   (open or resolved — a
 *                                                  report quotes a minor's
 *                                                  turn verbatim and must not
 ...
 *     rate } … }, topErrors } to today's DAU / messages / lessons / cost
 *     (the last perDay row), yesterday's cost (the row before it), and the
 *     window's WAU, cost, cost/WAU, abandoned count and retention rates.
 *     Never throws on a partial object — missing pieces come out undefined
 *     and render as "n/a".
 ...
 *     route. Canonical `stats` shape (every field is optional and renders as
 *     "n/a" when absent, so the digest degrades instead of throwing):
 *       {
 *         dau, wau,                          // distinct active sessions, today / 7d
 *         userMessages,                      // user-role messages today
 *         lessons: { started, completed,     // today
 *                    abandoned },            // over the 7-day window (a start
 *                                            // needs 24 h of silence to count,
 *                                            // so a today figure is always 0)
 *         cost: { today, yesterday, week },  // USD from the usage ledger
...
function digestStatsFromAdminStats(stats) {
  ...
  const yesterday = perDay.at(-2);
  const r = s.retention && typeof s.retention === 'object' ? s.retention : {};
  const rate = (d) => (d && typeof d === 'object' ? (d.rate ?? null) : d);
  return {
    dau: today.dau,
    wau: s.wau,
    userMessages: today.userMessages,
    lessons: { started: today.lessonsStarted, completed: today.lessonsCompleted, abandoned: s.lessonsAbandoned },
    cost: { today: today.costUsd, yesterday: yesterday ? yesterday.costUsd : undefined, week: s.costUsdWindow },
...
function formatDigest(stats, { day } = {}) {
  ...
  const lines = [
    `**Mercurius daily digest${day ? ` — ${day} (UTC)` : ''}**`,
    ...
    `Cost: yesterday ${fmtUsd(costYesterday)} · today so far ${fmtUsd(costToday)}`,
...
function createScheduler(deps = {}) {
  ...
  async function runDigest(t) {
    ...
    let text;
    try {
      text = formatDigest(digestStatsFromAdminStats(stats), { day: today });
    } catch (err) {
