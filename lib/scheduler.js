 ...
 *       getAdminStats({ days })         → Promise<stats>  (see formatDigest)
 ...
 *         reportsBefore(ts, { resolvedOnly: true })  // else counts as 0
 *         usageBefore(ts)
 ...
 *         getAdminStats({ days: 1 }) → notify('digest', formatDigest(stats),
 ...
 *           REPORT_RETENTION_DAYS         = 180   (resolved reports only)
 ...
 *     route. Canonical `stats` shape (what getAdminStats should return;
 *     every field is optional and renders as "n/a" when absent, so the
 *     digest degrades instead of throwing):
 *       {
 *         dau, wau,                          // distinct active sessions, 1d / 7d
 *         userMessages,                      // user-role messages in the window
 *         lessons: { started, completed, abandoned },
...
const PURGES = [
  ...
  { label: 'reports', env: 'REPORT_RETENTION_DAYS', def: 180, unit: DAY_MS, fn: 'reportsBefore', args: [{ resolvedOnly: true }], note: 'resolved' },
...
function formatDigest(stats, { day } = {}) {
  ...
  const d1 = pick(s, 'retention.d1', 'retentionD1', 'd1');
  const d7 = pick(s, 'retention.d7', 'retentionD7', 'd7');
  const errors = topErrors(s, 3);

  const lines = [
    `**Mercurius daily digest${day ? ` — ${day} (UTC)` : ''}**`,
    `DAU ${fmtInt(dau)} · user messages ${fmtInt(userMessages)}`,
    `Lessons: ${fmtInt(started)} started · ${fmtInt(completed)} completed · ${fmtInt(abandoned)} abandoned`,
    `Cost: yesterday ${fmtUsd(costYesterday)} · today so far ${fmtUsd(costToday)}`,
    `WAU ${fmtInt(wau)} · cost/WAU ${fmtUsd(costPerWau)} · D1 ${fmtPct(d1)} · D7 ${fmtPct(d7)}`,
...
function createScheduler(deps = {}) {
  ...
  async function runDigest(t) {
    ...
    let stats;
    try {
      stats = await deps.getAdminStats({ days: 1 });
    } catch (err) {
    ...
    let text;
    try {
      text = formatDigest(stats, { day: today });
    } catch (err) {
