...
// ── Standby gamification (mascot: Mercury) ──
// Only ever incremented from the flag-gated /api/progression routes, so these
// stay at zero unless GAMIFICATION_ENABLED is on. They track the *engagement*
// layer (XP/Level) — never rank, which is a separate, un-instrumented track.
const gamificationEventsTotal = new promClient.Counter({
  name: 'gamification_events_total',
  help: 'Progression events by reason + outcome status (awarded/capped_day/capped_session/idempotent/diminished_to_zero)',
  labelNames: ['reason', 'status'],
  registers: [register],
});

const gamificationXpAwardedTotal = new promClient.Counter({
  name: 'gamification_xp_awarded_total',
  help: 'Total XP points awarded, by reason',
  labelNames: ['reason'],
  registers: [register],
});

// ── Model spend + capacity (incremented only from lib/claudeCall.js) ──
// `kind` is one of input | output | cache_read | cache_write so a single
// counter family answers "what did caching actually save" per route.
const anthropicTokensTotal = new promClient.Counter({
  name: 'anthropic_tokens_total',
  help: 'Tokens consumed by upstream model calls, by route, model, and token kind',
  labelNames: ['route', 'model', 'kind'],
  registers: [register],
});

const anthropicCostUsdTotal = new promClient.Counter({
  name: 'anthropic_cost_usd_total',
  help: 'Estimated USD cost of upstream model calls (lib/pricing rates), by route and model',
  labelNames: ['route', 'model'],
  registers: [register],
});

const inflightStreams = new promClient.Gauge({
  name: 'inflight_model_calls',
  help: 'Model calls currently open (streams + non-streaming requests)',
  registers: [register],
});

const quotaRejectionsTotal = new promClient.Counter({
  name: 'quota_rejections_total',
  help: 'Requests refused by a daily quota, in-flight cap, budget, or kill switch, by scope',
  labelNames: ['scope'],
  registers: [register],
});

const lessonEventsTotal = new promClient.Counter({
  name: 'lesson_events_total',
  help: 'Curriculum lesson lifecycle events (start / turn / complete)',
  labelNames: ['event'],
  registers: [register],
});
...
/**
 * Mount GET /metrics on the given Express app. Any extra arguments are
 * Express middleware run before the handler (server.js passes the admin
 * limiter + password check so the scrape endpoint is not public).
 */
function mount(app, path = '/metrics', ...middleware) {
  app.get(path, ...middleware, async (_req, res) => {
...
module.exports = {
  ...
  anthropicCostUsdTotal,
  inflightStreams,
  quotaRejectionsTotal,
  lessonEventsTotal,
  gamificationEventsTotal,
  gamificationXpAwardedTotal,
};
