 ...
 *
 * There is deliberately no memory or performance section: the server keeps
 * no per-student profile (the LLM-extracted `student_memory` was removed —
 * undisclosed, never surfaced to the student, and it doubled per-turn cost).
 */
function buildRuntimeContext({ mode, responseMode, currentDate, meeting, blog }) {
  const parts = [
    '<runtime>',
    `  <mode>${mode}</mode>`,
