...
**Trust + ops rails.** Content reports (`POST /api/report`, reason + preceding user turn + context) land in an admin review queue (`GET /api/admin/reports`, `POST /api/admin/reports/:id/resolve`) and post to Discord via `lib/reportWebhook.js`. Lesson lifecycle rows (`lesson_events`: start / turn / complete) are written from the chat handler; `GET /api/admin/stats?days=7` is the founder's weekly numbers (DAU/WAU, lessons, cost, D1/D7) plus live rails state. `lib/scheduler.js` runs in-process (one replica, no cron): a daily Discord digest and the retention sweep (`*_RETENTION_*` env; open reports and `curriculum_progress` are never purged). Off under `NODE_ENV=test`.
